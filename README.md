...
#### 1. Используя наработки и документацию EthernNet/IP попробовать настроить коммуникацию между ПЛК и ПК.  
#### 2. Взаимодействуя с командой отладки линии 316, протестировать полный цикл работы установки и программы СУ.  
#### 3. Разместить в Kubernetes программу СУ, а также сервер OPCUA и веб-интерфейс.  
#### 4. Заменить ПЛК Siemens S7-1200 на I/O модули, работающие по протоколу EtherNet/IP "из коробки".  
#### 5. Как альтернативу EtherNet/IP можно попробовать реализовать связь по протоколу OPC UA over TSN. Он также является протоклом реального времени, для его реализации необходим промышленный TSN коммутатор.  
