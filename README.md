...
Смотри,а были созданы 2 программы. Первая из которых запускает OPCua сервер со всеми тегами из PLC. Получается, ааа... Также, ааа. В ней поднят или открыт клиент этого OPC сервера OPC клиент ПЛК. ааа... Который записывает все значения .. сейчас сформируем. записывает все значения тегов из контроллер (обновляет) в этот сервер (по задумке сервер должен быть развернут в Kubernetes). ааа.. Значения, получается, для этого ну этого клиента берутся из клиента номер 2 (клиент плк) аааа... Он с эм периодичностью сне помню точно... 
